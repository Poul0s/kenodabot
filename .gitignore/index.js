...
botcyb.on('message', async message =>  {
    ...
    message.guild.channels.find("name", "crocs-logs").send("@[Modérateur]")
    }
})
...
botcyb.on('message', async message =>  {
    ...
    if(pingbl) {
    message.guild.channels.find("name", "crocs-logs").send(`${auth} a envoyé un lien discord dans ${message.channel.name} (@[Modérateur])`)
    }
}
}
})
